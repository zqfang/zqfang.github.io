...
**Jensen 不等式**:   
$f$为凸函数, 则$E[f(X)] \geq f(E[X])$。当且仅当x是常数时，$E[f(x)]=f(E[x])$。  
$f$为凸函数, $\forall_{x \in \mathbb{R}}, f^{\prime \prime}(x) \geq 0$, 当x为向量，如果其hessian矩阵H是半正定的($H \geq 0$)。  
...
这里，$Q\left(z_{i}\right)$是$z_{i}$的函数， 且$\sum_{z_{i}} Q\left(z_{i}\right)=1$。   
由数学期望$E_{x \sim p}[g(X)]=\sum_{x} g(x) p(x)$，上式可以理解为: p(x)对应$Q\left(z_{i}\right)$, g(x)对应$\log \frac{p\left(x_{i}, z_{i} \| \theta\right)}{Q\left(z_{i}\right)}$表示$z_{i}$的函数。    
log为凸函数，根据Jensen不等式：
